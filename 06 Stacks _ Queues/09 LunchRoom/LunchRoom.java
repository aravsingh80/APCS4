...
public class LunchRoom
{
   ...
   public static int frTotal = 0;
   public static int frLongest = 0;
   public static int frTotalTime = 0;
   public static int soTotal = 0;
   public static int soLongest = 0;
   public static int soTotalTime = 0;
   public static int juTotal = 0;
   public static int juLongest = 0;
   public static int juTotalTime = 0;
   public static int seTotal = 0;
   public static int seLongest = 0;
   public static int seTotalTime = 0;
   public static int[] served, lWait, aWait;
   ...
   public static void display(int t, PriorityQueue<Customer> q)
   {
      if(q.size() > 1)
      {
         ...
         outfile.println(t + ":" + "[" + s2);
      }
      else
         displayTimeAndQueue(q, t);
      if(!(q.isEmpty()))
         outfile.println("  Service Area #1: " + q.peek().getTime() + ":" + q.peek().getGrade() + ":" + sTime);
      else
         outfile.println("  Service Area #1: -1:-1");
      outfile.println();
   }
    ...
    ******************/ 
      ...
      public Customer(String g, int a, int s, boolean l)
      {
         grade = g;
         arr = a;
         serve = s;
      ...
      public void setTime(int ar)
      {
         arr = ar;
      }
      public void setServeTime(int se)
      {
         serve = se;
      }
      ...
      public int compareTo(Customer c)
      {
         if(c.inLine())
            return 1;
         else if(grade.equals("Se") && !(c.getGrade().equals("Se")))
            return -1;
         else if(grade.equals("Ju") && (c.getGrade().equals("So") || c.getGrade().equals("Fr")))
            return -1;
         else if(grade.equals("So") && c.getGrade().equals("Fr"))
            return -1;
         else if(grade.equals(c.getGrade()))
         {
            if(arr < c.getTime())
               return -1;
            else 
   ...
   public static void LunchRoom()
   {
      ...
      *************************/
      ...
      for(int x = 0; x < TIME; x++)
      {
         ...
         {
            String s = grade();
            Customer c = new Customer(s, x, timeToOrderAndBeServed(), false);
            queue.add(c);
            outfile.println("New Customer " + c);
            outfile.println();
            switch(s)
            {
               case("Fr"):
         ...
         if(sTime == 0)
         {
            int min = x - queue.peek().getTime() + 1;
            switch(queue.peek().getGrade())
            {
               case("Fr"):
                  frTotalTime = frTotalTime + min;
                  if(min >= frLongest)
                     frLongest = min;
                  break;
               case("So"):
                  soTotalTime = soTotalTime + min;
                  if(min >= soLongest)
                     soLongest = min;
                  break;
               case("Ju"):
                  juTotalTime = juTotalTime + min;
                  if(min >= juLongest)
                     juLongest = min;
                  break;
               case("Se"):
                  seTotalTime = seTotalTime + min;
                  if(min >= seLongest)
                     seLongest = min;
                  break;
            }
            outfile.println("Customer# " + queue.peek().getTime() + ":" + queue.peek().getGrade() + " leaves and his/her total wait time is " + min);
            outfile.println();
            queue.remove();
            if(!(queue.isEmpty()))
            {
      ...
      while(!(queue.isEmpty()))
      {
         ...
         int min = t - queue.peek().getTime() + 1;
         switch(queue.peek().getGrade())
         {
            case("Fr"):
               frTotalTime = frTotalTime + min;
